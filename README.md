...
| ------ | ---------- | --------- | ------------- |
| `token` | no | `${{ github.token }}` | Token used to call the API. **The default `GITHUB_TOKEN` will not work** — the runners-list endpoint requires admin access to the repo/org, which `GITHUB_TOKEN` never has. Provide a PAT with `repo` scope (repo scope) or `admin:org` scope (org scope), or an equivalent fine-grained PAT/App token (see [Permissions](#permissions)). |
...
**The default `GITHUB_TOKEN` does not work for either scope.** GitHub's
self-hosted runner endpoints (`/repos/{owner}/{repo}/actions/runners` and
`/orgs/{org}/actions/runners`) require the caller to have **admin access** to
the repository or organization — this is a different, stricter requirement
than the `actions: read`/`write` workflow permission, which only covers
managing workflow runs. `GITHUB_TOKEN` can never satisfy the admin-access
check, so calling these endpoints with it returns a `404 Not Found` (GitHub
masks the permission failure as "not found" rather than a 403).

You must supply your own token via the `token` input, generated by an
account with admin rights on the target repo (for `scope: repo`) or
organization (for `scope: org`):

- **Classic PAT:** `repo` scope for repo scope, `admin:org` scope for org scope.
- **Fine-grained PAT:** "Administration" repository permission (read) for
  repo scope, or the "Self-hosted runners" organization permission (read) for
  org scope.
